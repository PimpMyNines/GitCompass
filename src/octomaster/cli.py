#!/usr/bin/env python3
"""Command-line interface for OctoMaster."""
...
@click.option('--repo', '-r', required=True, help='Repository in format owner/repo')
@click.option('--output', '-o', help='Output file path for the report (if not specified, prints to console)')
...
def version():
    """Show the OctoMaster version."""
    from octomaster import __version__
    click.echo(f"OctoMaster version {__version__}")
